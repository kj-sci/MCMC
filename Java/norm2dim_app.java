...
/*
 * Sampling from {y>=0.5*x^2+3}
 * pdf = A * exp(-(x^2-2*beta*x*y+y^2)/2)
 *
 */
public class norm2dim_app extends Applet implements Runnable{
	// beta
	double beta;
	// step size
	double sigma;
	...
	public void init() {
		...
		mcmc_hd = new norm2dim_metropolis(beta, sigma);
	...
	public void run() {
		for (loop_siml=0; loop_siml<num_siml; loop_siml++) {
			...
			state = mcmc_hd.get_state();
			System.err.println(loop_siml+": ("+state[0]+","+state[1]+")");
